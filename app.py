...
price_chart_data = pd.concat([
    hist.sort_values("Date").tail(60)[["Date", "Close"]].rename(columns={"Close": "Price"}),
    pred[["Date", "Predicted_Price"]].rename(columns={"Predicted_Price": "Price"})
])
price_chart_data["Type"] = ["Historical"] * 60 + ["Forecast"] * len(pred)
